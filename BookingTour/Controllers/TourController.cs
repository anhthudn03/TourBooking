...
namespace BookingTourAPI.Controllers
{
    ...
    public class TourController : ControllerBase
    {
        ...
        [Authorize]
        public async Task<IActionResult> ChangeStatusTour([FromRoute]int tourId)
        {
            int currentUserId = int.Parse(HttpContext.User.FindFirstValue(JwtRegisteredClaimNames.Sid));

            var tour = await _tourBusiness.UpdateStatusTour(tourId, currentUserId);
            if (tour)
            {
        ...
        public async Task<IActionResult> DeleteTourImage([FromRoute] int imageId)
        {
            var tourImage = await _tourBusiness.DeleteImageTour(imageId);
            if (!tourImage)
            {
                return BadRequest();
            }
