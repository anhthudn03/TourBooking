...
namespace BusinessLogic.Business
{
    public class UserBusiness
    {
        ...
        {
            var user = _repository.FindByCondition(u => u.TenDangNHap == username.Trim()).FirstOrDefault();
            if (user == null)
            {
                throw new NotFoundException("User Not Found");
            }
        ...
        public async Task<UpdateUserModel> UpdateUser(int id, UpdateUserModel model)
        {
            ...
            userExist.NgaySinh = model.NgaySinh;
            userExist.TenDangNHap = model.TenDangNhap;
            userExist.MatKhau = model.MatKhau;
            userExist.Email = model.Email;
            userExist.SDT = model.SDT;
            userExist.HoVaTen = model.HoVaTen;
            userExist.DiaChi = model.DiaChi;
            userExist.NgaySua = DateTime.UtcNow;
