...
namespace FSAM.BusinessLogic.Services
{
    ...
    public class AuthBusiness
    {
        ...
        public async Task<LoginResponseModel> Login(LoginModel login)
        {
            ...
            var userExist = await _userRepository.FindByCondition(u => u.TenDangNHap == login.TenDangNhap).FirstOrDefaultAsync();
            ...
            if (userExist is null)
            {
                ...
                {
                    Success = false,
                    Message = "TenDangNhap not exist"

                };

            }
            string storedPassword = userExist.MatKhau;
        ...
        public async Task<bool> Register(CreateUserModel createUserModel)
        {
            try
            {
                var userExist = await _userRepository.GetAll().Where(u => u.TenDangNHap == createUserModel.TenDangNHap).FirstOrDefaultAsync();
                if (userExist != null)
                {
                    throw new BadRequestException("User has exist");
                }
                string passwordHash = BCrypt.Net.BCrypt.HashPassword(createUserModel.MatKhau);

                var newUser = new User
                {
                    TenDangNHap = createUserModel.TenDangNHap,
                    Email = createUserModel.Email,
                    DiaChi = createUserModel.DiaChi,
                    NgaySinh = createUserModel.NgaySinh,
                    HoVaTen = createUserModel.HoVaTen,
                    MatKhau = passwordHash,
                    SDT = createUserModel.SDT,
        ...
        public async Task<TokenModel> GenerateJwtToken(UserModel userModel)
        {
            ...
            var tokenDescription = new SecurityTokenDescriptor
            {
                Subject = new ClaimsIdentity(new[]
                {
                    ...
                    new Claim(ClaimTypes.Role,  user.ChucVu)
        }),
                Expires = expries,
